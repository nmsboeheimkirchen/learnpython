...
switch ($argv[1] ?? '') {
    case 'transfer-expire':
        $db->prepare('UPDATE student_transfers SET expires_at=1 WHERE id=?')->execute([$input['id']]);break;
    case 'transfer-inspect':
        $q=$db->prepare('SELECT id,mode,expires_at FROM student_transfers WHERE user_id=? ORDER BY id');$q->execute([$input['id']]);
        echo json_encode($q->fetchAll());break;
    case 'transfer-user-snapshot':
        // Only non-secret metadata + digest; never print password hashes or source.
        $q=$db->prepare('SELECT u.email,u.display_name,u.password_hash,u.active,e.confirmed FROM users u LEFT JOIN email_confirmations e ON e.user_id=u.id WHERE u.id=?');$q->execute([$input['id']]);
        $credential=$q->fetch();
        $q=$db->prepare('SELECT * FROM learning_states WHERE user_id=?');$q->execute([$input['id']]);
        $q2=$db->prepare('SELECT class_id FROM users WHERE id=?');$q2->execute([$input['id']]);
        echo json_encode(['preservedHash'=>hash('sha256',json_encode([$credential,$q->fetch()])), 'primary'=>$q2->fetchColumn(),'classes'=>array_column(AgentPy\accountClasses($db,$input['id']),'id')]);break;
    case 'test-transfer-migration':
        $snapshot=function()use($db){$out=[];foreach(['users','classes','learning_states','teachers','teacher_classes','class_teachers','class_memberships','class_namespaces','email_confirmations']as $table){$rows=array_map(fn($row)=>json_encode($row),$db->query('SELECT * FROM '.$table)->fetchAll());sort($rows);$out[$table]=$rows;}return $out;};
        $before=$snapshot();$db->exec('DROP TABLE student_transfers');$db->exec('DELETE FROM schema_migrations WHERE version=9');
        AgentPy\migrateTransfers($db);AgentPy\migrateTransfers($db);
    ...
    case 'init-v2-only':
        ...
        foreach (['student_transfers','class_namespaces','teacher_mail_jobs','teacher_invitations','class_teachers','superadmins','class_memberships','email_confirmations','invitation_secrets','teacher_classes','teachers','teacher_audit','recovery_mail_jobs','password_resets','auth_epochs','mail_jobs', 'pending_registrations', 'class_invitations', 'class_registration', 'mail_attempts', 'mail_dispatch_lock'] as $table)
    ...
    case 'test-profile-migration':
        ...
        foreach (['student_transfers','class_namespaces','invitation_secrets','teacher_classes','teachers','teacher_audit','recovery_mail_jobs','password_resets','auth_epochs','mail_jobs', 'pending_registrations', 'class_invitations', 'class_registration', 'mail_attempts', 'mail_dispatch_lock', 'write_receipts', 'learning_states', 'users', 'classes', 'schema_migrations', 'login_limits'] as $table)
            $db->exec('DROP TABLE IF EXISTS ' . $table);
