...
for (const engine of ['login-chromium', 'login-webkit']) {
    ...
    for (const name of ['student-a', 'student-b', 'shell-student', 'recovery-student','polish-student','verification-student','mode-student']) manage('create-user', { email: `${name}-${engine}@example.test`, password: 'Synthetic-browser-password-123!', name, classId: classInfo.id });
    for(const teacherName of ['teacher','membership-teacher']){
