...
for (const engine of ['login-chromium', 'login-webkit']) {
    ...
    for(const teacherName of ['teacher','membership-teacher','admin']){
    const teacher=JSON.parse(manage('create-user',{email:`${teacherName}-${engine}@example.test`,password:'Synthetic-browser-password-123!',name:'Michael Fixture',classId:classInfo.id}));
    ...
    if(teacherName==='admin'){const result=spawnSync(php,[...phpArgs,'tests/backend-fixture.php','admin-grant'],{cwd:root,env,input:JSON.stringify({id:teacher.id}),encoding:'utf8',windowsHide:true});if(result.status!==0)throw Error('Admin fixture setup failed');}
    }
}
