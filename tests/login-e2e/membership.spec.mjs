...
test('same-domain signup, teacher confirmation/correction, logout and shared-class deletion preserve progress',async({page,browser})=>{
    ...
    try{
        ...
        await expect(lesson(pupil).locator('html')).toHaveClass(/account-invalidated/);
        await pupil.reload();await login(pupil,corrected);
        ...
        await expect(page.locator('.teacher-preserved')).toContainText(corrected);
        await expect(page.locator('.teacher-preserved')).toContainText(b.name);
        await expect(page.locator('.teacher-preserved')).toContainText('bleiben erhalten');
        ...
        await expect(lesson(page).getByRole('cell',{name:'5 %',exact:true})).toBeVisible();
        await lesson(page).getByRole('button',{name:'Klasse löschen',exact:true}).click();
        await expect(page.locator('.teacher-preserved')).toHaveCount(0);
        await page.getByRole('button',{name:'Abbrechen',exact:true}).click();
