...
try {
    ...
    const php = `ini_set("display_errors","0"); try { require "${privateRoot}/releases/${releaseId}/app/src/bootstrap.php"; require_once "${privateRoot}/releases/${releaseId}/app/src/account-management.php"; $input=json_decode(stream_get_contents(STDIN,2048),true,10,JSON_THROW_ON_ERROR); AgentPy\\exactFields($input,["email","password","name","className"]); $db=AgentPy\\database(AgentPy\\config()); if($db->query("SELECT DATABASE()")->fetchColumn()!=="u535472856_agentpy") throw new RuntimeException(); $name=AgentPy\\accountLabel($input["className"]); $q=$db->prepare("SELECT id FROM classes WHERE name = ?"); $q->execute([$name]); $classId=$q->fetchColumn(); if($classId===false)$classId=AgentPy\\createClass($db,["name"=>$name])["id"]; unset($input["className"]); $input["classId"]=$classId; echo json_encode(AgentPy\\createAccount($db,$input),JSON_THROW_ON_ERROR); } catch(Throwable $e){fwrite(STDERR,"ACCOUNT_PROVISION_FAILED");exit(1);}`.replaceAll('\\\\', '\\');
    let profile;
    try {
